...
def game_start():
    ...
    score = 0
    health = 100
    player_start_y = int(((mod_dimensions_y / 2) + 40) - 10)  # mod_dimensions_y is the height of the window,
    # divided by 2, adding 40 to correctly place, then subtracting 10 for finer adjustments.
    ...
    info_container = tk.Frame(background1, height=80, width=160, bg="white", highlightbackground="black",
                              highlightthickness=2)

    health_counter = tk.Label(info_container, text=("Hit Points: " + str(health)), bg="white",
                              font=("Kitchen Knight", 12))
    ...
    info2 = background1.create_text(((mod_dimensions_x - 100), 10), text="Press the Escape key to quit.",
                                    font=("Kitchen Knight", 12))
...
def move_player(event):
    try:
        ...
        # if k == "a" and ((player_x > 0) and (((player_x-10) not in obstacle_list_x) or (player_y not in
        # obstacle_list_y))): player_x = player_x - 10

        # if k == "d" and ((player_x < (mod_dimensions_x-10)) and (((player_x+10) not in obstacle_list_x) or (
        # player_y not in obstacle_list_y))): player_x = player_x + 10
        ...
        if k == "d" and ((player_x < (mod_dimensions_x - 10)) and (
                ((player_x + 10) not in obstacle_list_x) or (player_y not in obstacle_list_y))):
            player_x = player_x + 10
...
start_button = tk.Button(start_background, text="Start Game", fg="blue", command=game_start,
                         font=("Kitchen Knight", 12))
info1 = tk.Label(start_background, text="To quit the game, press the Escape key.", fg="blue", bg="#888888",
                 font=("Kitchen Knight", 12))
